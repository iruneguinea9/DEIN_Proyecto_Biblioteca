...
public class AniadirAlumnoController implements Initializable {
	...
	private String comprobarCampos() {
		...
		if (dniTxt.getText().toString().length() != 9)
			str += bundle.getString("err6") + "\n";
		try {
			Integer.parseInt(dniTxt.getText().toString().substring(0, 8));
			if (!dniTxt.getText().toString().substring(8).toUpperCase().matches("[A-Z]*"))
				str += bundle.getString("err9") + "\n";
			
		} catch (NumberFormatException e) {
			str += bundle.getString("err8") + "\n";
